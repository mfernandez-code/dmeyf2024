...
FErf_attributes_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$lgb_param <- list(
    ...
    boosting = "rf",
    bagging_fraction = 0.5,#( 1.0 - 1.0/exp(1.0) ),
    bagging_freq = 3.0,
...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    bagging_fraction = 0.50, # 0.0 < bagging_fraction <= 1.0
    pos_bagging_fraction = 1.0, # 0.0 < pos_bagging_fraction <= 1.0
    neg_bagging_fraction = 0.60, # 0.0 < neg_bagging_fraction <= 1.0
    is_unbalance = FALSE, #
...
wf_SEMI_ago_orden333 <- function( pnombrewf )
{
